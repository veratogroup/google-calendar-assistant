...
// Scheduling rules (override via env if desired)
const RULES = {
  ...
  BUFFER_MIN: parseInt(process.env.BUFFER_MIN || '10', 10),              // buffer around events
  MIN_NOTICE_MIN: parseInt(process.env.MIN_NOTICE_MIN || '120', 10)      // 2 hours notice
};
...
function requireActionToken(req, res, next) {
  ...
  if (!token || token !== process.env.ACTION_TOKEN) return res.status(401).json({ error: 'Unauthorized action' });
  next();
}
...
async function freeBusy(timeMin, timeMax) {
  const { data } = await calendar.freebusy.query({
    requestBody: { timeMin, timeMax, items: [{ id: CAL_ID }] }
  });
  return data?.calendars?.[CAL_ID]?.busy || [];
}

function withinWorkHours(dateUtc, tz) {
...
// ---------- Vonage Voice (PUBLIC) ----------
function verifyVonageSignature(_req) {
  // TODO: Implement HMAC signature check if VONAGE_SIGNATURE_SECRET is set.
  // For now we allow if not configured.
  return !process.env.VONAGE_SIGNATURE_SECRET;
}

// Inbound answer URL -> return NCCO
...
// Handle voice input (demo intents)
app.post('/vonage/input', async (req, res) => {
  ...
  try {
    if (/\bhelp\b/.test(utter)) {
      ...
      textOut = firstFew.length ? `Here are some times: ${firstFew.join('. ')}. Say book and then say the number you want.` : 'No openings left today. Say book to try another day.';
...
// ---------- Public token‑gated summaries (NO API key) ----------
app.get('/summary/next-week', async (req, res) => {
  try {
    const token = req.query.token;
    if (!token || token !== process.env.SUMMARY_TOKEN) return res.status(401).json({ error: 'Unauthorized' });
    const { timeMin, timeMax } = nextWeekWindow();
    const items = await listEvents(timeMin, timeMax);
    const sanitized = items.map(e => ({ id: e.id, when: { start: e.start, end: e.end }, title: e.summary || 'Busy' }));
    res.json({ timeMin, timeMax, items: sanitized });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Flexible summary: /summary?start=YYYY-MM-DD&end=YYYY-MM-DD[&tz=America/Chicago]&token=...
...
// ---------- Everything below requires API key ----------
...
// Read
app.get('/events/next-week', async (_req, res) => {
  try {
    const { timeMin, timeMax } = nextWeekWindow();
    const items = await listEvents(timeMin, timeMax);
    res.json(items);
...
app.post('/events', async (req, res) => {
  try {
    ...
    const resource = { summary, description, location, start: { dateTime: startISO }, end: { dateTime: endISO }, attendees };
...
app.patch('/events/:id', async (req, res) => {
  try {
    ...
    const params = { calendarId: CAL_ID, eventId: id, resource, sendUpdates: 'all' };
...
app.get('/action/create', requireActionToken, async (req, res) => {
  try {
    ...
    const { data } = await calendar.events.insert({
      calendarId: CAL_ID,
      resource: { summary, description, location, start: { dateTime: startISO }, end: { dateTime: endISO } },
...
app.get('/action/reschedule', requireActionToken, async (req, res) => {
  try {
    ...
    const { data } = await calendar.events.patch({
      calendarId: CAL_ID, eventId: id, resource, conferenceDataVersion: 1, sendUpdates: 'all'
    });
    res.json({ ok: true, id: data.id });
  } catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.post('/jobs/next-week-summary', async (_req, res) => {
  try {
    const { timeMin, timeMax } = nextWeekWindow();
    const items = await listEvents(timeMin, timeMax);
